...
import { ResolvedEntity } from "../types/analytics.js";
...
export class TarkovLogsInsights {
  ...
  async compute(): Promise<Insights> {
  ...
  private buildTimelines(): SessionTimeline[] {
    const bySession = new Map<string, SessionTimeline>();
    for (const res of this.results) {
      const sessionId =
        res.meta.sessionPrefix ??
      ...
      const ts = (event: AnyLogEvent) => event.timestamp;
    ...
    for (const session of bySession.values()) {
      session.startupDurationMs = duration(
        session.startedAt,
        session.firstBackendAt
      );
      session.matchmakingDurationMs = duration(
        session.firstMatchEventAt,
  ...
  private buildMatching(): MatchingInsight {
    ...
    for (const res of this.results.filter((r) => r.logType === "application")) {
      ...
      for (const e of res.events) {
        if (e.eventFamily === "matchmaking") {
          const g = (e.fields as any)?.groupId;
          if (g) agg.groupId = agg.groupId ?? g;
          if (!agg.startedAt) agg.startedAt = e.timestamp;
    ...
    const sessions: MatchingInsight["sessions"] = [];
    for (const agg of bySession.values()) {
      if (!agg.startedAt && !agg.preparedAt && !agg.runnedAt) continue;
      const durationMs = duration(
        agg.startedAt,
        agg.runnedAt ?? agg.preparedAt
      );
      sessions.push({
    ...
    const averageDurationMs =
      sessions
        ...
        .reduce(
          (acc, s, _, arr) => acc + (s.durationMs ?? 0) / arr.length,
          0
        ) || undefined;
    return { sessions, averageDurationMs };
  }

  private buildStartup(): StartupInsight {
    // Use a Map to deduplicate sessions by sessionId
    const bySession = new Map<
      string,
      { sessionId: string; startedAt?: string; firstBackendAt?: string }
    >();
    ...
    const sessions: StartupInsight["sessions"] = [];
  ...
  private buildErrors(): ErrorInsight {
  ...
  private buildInventory(): InventoryInsight {
    ...
    for (const res of this.results.filter((r) => r.logType === "inventory")) {
      for (const e of res.events) {
        totalRejections += 1;
        const op = (e.fields as any)?.operationType;
        const code = (e.fields as any)?.code;
  ...
  private buildConnectivity(): ConnectivityInsight {
    const byAddress: Record<string, { connect: number; disconnect: number; timeout: number }> = {};
    let totalConnections = 0;
    let totalDisconnects = 0;
    let totalTimeouts = 0;
    for (const res of this.results.filter(
      (r) => r.logType === "network-connection"
    )) {
      for (const e of res.events) {
        const addr = (e.fields as any)?.address ?? "unknown";
  ...
  private async buildQuests(): Promise<{
    list: QuestInsight[];
    items: Record<string, ResolvedEntity>;
    traders: Record<string, ResolvedEntity>;
  }> {
    const quests: Record<string, QuestInsight> = {};
        ...
        if (!quests[questId]) {
          ...
          if (resolved) {
            ...
            if (resolved.traderId) {
              quests[questId].traderName = await this.resolveTraderName(
                resolved.traderId,
                traders
              );
            }
  ...
  private async resolveQuest(
    id: string
  ): Promise<{ name?: string; traderId?: string } | undefined> {
  ...
  private async resolveTraderName(
    traderId: string,
    traders: Record<string, ResolvedEntity>
  ...
  private upsertResolved(
    target: Record<string, ResolvedEntity>,
    id: string,
