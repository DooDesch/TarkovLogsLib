...
const HEADER_REGEX =
  /^(?<timestamp>\d{4}-\d{2}-\d{2} [^|]+)\|(?<version>[^|]*)\|Error\|objectPool\|(?<assetId>[^|]+)\|(?<message>.*)$/;

export class ObjectPoolLogsParser implements LogParser {
  canParse(fileName: string, sampleContent?: string): boolean {
    const lowered = fileName.toLowerCase();
    return (
      lowered.includes("objectpool_") ||
  ...
  parse(content: string, filePath?: string): ParsedLogResult<ObjectPoolLogEvent> {
    ...
    const events: ObjectPoolLogEvent[] = groups.map(({ match, continuation }) => {
      const { timestamp = "", version = "", assetId = "", message = "" } =
