...
const HEADER_REGEX =
  /^(?<timestamp>\d{4}-\d{2}-\d{2} [^|]+)\|(?<version>[^|]*)\|Info\|backendCache\|BackendCache\.Load File name: (?<rest>.*)$/;

export class BackendCacheLogsParser implements LogParser {
  canParse(fileName: string, sampleContent?: string): boolean {
    const lowered = fileName.toLowerCase();
    return (
      lowered.includes("backendcache_") ||
  ...
  parse(
    content: string,
    filePath?: string,
  ): ParsedLogResult<BackendCacheLogEvent> {
    const groups = groupByHeader(content, HEADER_REGEX);
    const events: BackendCacheLogEvent[] = groups.map(({ match }) => {
      const { timestamp = "", version = "", rest = "" } =
...
function classify(rest: string): BackendCacheLogEvent["fields"] {
