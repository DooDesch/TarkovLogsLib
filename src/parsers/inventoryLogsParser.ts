...
const HEADER_REGEX =
  /^(?<timestamp>\d{4}-\d{2}-\d{2} [^|]+)\|(?<version>[^|]*)\|Error\|inventory\|(?<message>.*)$/;

export class InventoryLogsParser implements LogParser {
  canParse(fileName: string, sampleContent?: string): boolean {
    const lowered = fileName.toLowerCase();
    return (
      lowered.includes("inventory_") ||
  ...
  parse(
    content: string,
    filePath?: string,
  ): ParsedLogResult<InventoryLogEvent> {
    const groups = groupByHeader(content, HEADER_REGEX);
    const events: InventoryLogEvent[] = groups.map(({ match, continuation }) => {
...
function parseContinuation(lines: string[]): InventoryLogEvent["fields"] {
  const result: InventoryLogEvent["fields"] = {};
